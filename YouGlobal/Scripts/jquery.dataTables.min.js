...
(function ($) {
    ...
    _oExt.oPagination = {
        two_button: {
            fnInit: function (oSettings, fnCallbackDraw) {
                ...
                if (!oSettings.bJUI) {
                    ...
                    oSettings.nPrevious = document.createElement("a"); oSettings.nNext = document.createElement("a");
            ...
            fnInit: function (oSettings, fnCallbackDraw) {
                ...
                var sList = ""; var iStartButton; var iEndButton; var oClasses = oSettings.oClasses; if (iPages < iPageCount) {
                    iStartButton = 1;
                    iEndButton = iPages
                } else {
                ...
                $("span", oSettings.nPaginateList).bind("mousedown", function () { return false }); $("span", oSettings.nPaginateList).bind("selectstart", function () {
                    ...
                    return false
                }); var nButtons = $("span", oSettings.anFeatures.p); var nStatic = [nButtons[0], nButtons[1], nButtons[nButtons.length - 2], nButtons[nButtons.length - 1]];
                $(nStatic).removeClass(oClasses.sPageButton + " " + oClasses.sPageButtonActive); if (iCurrentPage == 1) {
        ...
        if (iParse !== null && !isNaN(iParse)) { return "date" } return null
    ...
    $.fn.dataTable = function (oInit) {
        var _aoSettings = $.fn.dataTableSettings; function classSettings() {
            ...
            this.anFeatures = []; this.oLanguage = { sProcessing: "Processing...", sLengthMenu: "Show _MENU_ results / page", sZeroRecords: "No matching results found", sInfo: "_START_ to _END_ of _TOTAL_ results", sInfoEmpty: "No results found", sInfoFiltered: "(filtered from _MAX_ total entries)", sInfoPostFix: "", sSearch: "Search:", sUrl: "", oPaginate: { sFirst: "First", sPrevious: "Previous", sNext: "Next", sLast: "Last" } };
            ...
            this.iCookieDuration = 60 * 60 * 24; this.sAjaxSource = null; this.bAjaxDataGet = true; this.fnServerData = $.getJSON;
                    ...
                    for (i = 0; i < oSettings.aoData.length; i++) {
                        ...
                        for (var j = 0; j < oSettings.aoColumns.length; j++) {
                            if (oSettings.aoColumns[j].bVisible) {
                                if (oSettings.aoData[i] !== null && oSettings.aoData[i].nTr.getElementsByTagName("td")[j - iCorrector] == nNode) {
            ...
            if (oSettings.bInitialised === false) {
                ...
                $.getJSON(oSettings.sAjaxSource, null, function (json) {
                    ...
                    if (oSettings.oFeatures.bSort) { _fnSort(oSettings) } else {
                        oSettings.aiDisplay = oSettings.aiDisplayMaster.slice();
                        _fnCalculateEnd(oSettings); _fnDraw(oSettings)
            ...
            nWrapper.className = oSettings.oClasses.sWrapper; if (oSettings.sTableId !== "") {
            ...
            var sStdMenu = '<select size="1" ' + sName + '><option value="10">5</option><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select>';
            ...
            for (i = 0; i < iColumns; i++) {
                $(oSettings.aoColumns[i].nTh).removeClass(oClasses.sSortAsc + " " + oClasses.sSortDesc + " " + oClasses.sSortable)
            } if (oSettings.aaSortingFixed !== null) {
                aaSort = oSettings.aaSortingFixed.concat(oSettings.aaSorting)
                ...
                if (oSettings.aoColumns[i].bSortable && oSettings.aoColumns[i].bVisible) {
                    ...
                    iFound = -1; for (j = 0; j < aaSort.length; j++) {
                        if (aaSort[j][0] == i) {
                        ...
                        var sSpanClass; if (iFound == -1) { sSpanClass = oClasses.sSortJUI } else {
                ...
                var iClass = 1; for (i = 0; i < aaSort.length; i++) {
                    var iVis = _fnColumnIndexToVisible(oSettings, aaSort[i][0]);
                    if (iVis !== null) {
                        if (iClass <= 2) { $("td:eq(" + iVis + ")", nTrs).addClass(sClass + iClass) } else {
            ...
            var iLen = oSettings.aoData.length; for (var i = 0; i < iLen; i++) {
                if (oSettings.aoData[i] === null) {
                    aData.push(null)
                } else { aData.push(oSettings.aoData[i]._aData) }
            } return aData
            ...
            var iLen = oSettings.aoData.length; for (var i = 0; i < iLen; i++) {
                if (oSettings.aoData[i] === null) {
                    aNodes.push(null)
                } else { aNodes.push(oSettings.aoData[i].nTr) }
            } return aNodes
            ...
            var ROWSPAN = 2, COLSPAN = 3, TDELEM = 4; var i, j, k, iLen, jLen, iColumnShifted; var fnShiftCol = function (a, i, j) {
                ...
                fnAddRow(i); var iColumn = 0; var nTds = []; for (j = 0, jLen = nTrs[i].childNodes.length;
                j < jLen; j++) {
                    ...
                    var iRowspan = nTds[j].getAttribute("rowspan") * 1; if (!iColspan || iColspan === 0 || iColspan === 1) {
                        ...
                        aLayout[i][iColumnShifted] = (nTds[j].nodeName == "TD") ? TDELEM : nTds[j]; if (iRowspan || iRowspan === 0 || iRowspan === 1) {
                            for (k = 1;
            ...
            if (sId !== null) { oSettings.sTableId = sId; oSettings.sInstance = sId } else {
                ...
                if (typeof oInit.bJQueryUI != "undefined") {
                    oSettings.oClasses = _oExt.oJUIClasses; if (typeof oInit.sDom == "undefined") {
                        oSettings.sDomPositioning = '<"fg-toolbar ui-widget-header ui-corner-tl ui-corner-tr ui-helper-clearfix"lfr>t<"fg-toolbar ui-widget-header ui-corner-bl ui-corner-br ui-helper-clearfix"ip>'
                    }
